...
`SmartPortHandSocketTests.cs` and `SmartPortGripReachTests.cs` cover fitting newly copied sockets to mapped knuckle positions, nested helpers, preserved existing sockets, shorter-arm reach and unchanged bone lengths/additive rest. `SmartPortHandSocketMetadataTests.cs` checks compiled attachment parent names, including case normalization.

`Engine/SmartPortGripContactEngineTest.cs` tests Citizen → Catgirl with the actual M4A1 at **1×**, without scaling the character. Enable only `HR_SMART_PORT_CONTACT_PROJECT` and set `HR_SMART_PORT_GRIP_MODEL` to the fresh custom port. Supply the same gun dependencies as the visual check, plus the official Facepunch/sandbox `Assets/weapons/M4a1/m4a1_wm.prefab` as `<scratch>/m4a1_wm.prefab`. It uses the prefab's model mounting transform converted to native mesh units, identically for both characters—not a character-specific offset. It samples both hands and the active support-hand IK goal during idle/walk/run and attacks (maximum drift/error: 0.15 units), verifies actual foot motion and unchanged model scales, and captures both sides. Results are in `grip-contact-result.json`; captures are `gun-contact-*.png`. The older raw-mesh visual hook remains unchanged as a separate mounting diagnostic.
