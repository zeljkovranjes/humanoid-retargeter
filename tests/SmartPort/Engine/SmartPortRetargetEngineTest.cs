...
public static class SmartPortRetargetEngineTest
{
    static bool started;
    ...
    {
        var expected = Environment.GetEnvironmentVariable("HR_SMART_PORT_RETARGET_TEST_PROJECT");
        ...
        started = true;
        output = Path.Combine(expected, "smart-port-retarget-result.json");
        _ = Run();
    }
            ...
            foreach (var (style, speed) in new[] { (0, 100f), (4, 120f), (5, 240f) })
            {
                ...
                for (var frame = 0; frame < 100; frame++)
                {
                    ...
                    foreach (var bone in skinBones)
                    {
                        ...
                        maxStretch = MathF.Max(maxStretch, MathF.Abs(length - bindLength));
                        if (!float.IsFinite(length) || MathF.Abs(length - bindLength) > .15f)
                            throw new Exception($"Stretched skin bone {bone.Name}: bind={bindLength}, animated={length}");
                    }
