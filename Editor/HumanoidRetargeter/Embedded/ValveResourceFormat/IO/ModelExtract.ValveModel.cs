...
using static HumanoidRetargeterVrf.IO.KVHelpers;
...
partial class ModelExtract
{
    ...
    static void AddBoneConstraintProperty<T>(KVObject sourceObject, KVObject targetObject, string sourceName, string targetName)
    {
        if (sourceObject.ContainsKey(sourceName))
        {
            if (typeof(T) == typeof(global::System.Numerics.Quaternion))
            {
                ...
                var rot = new global::System.Numerics.Quaternion(value[0], value[1], value[2], value[3]);
            ...
            {
                var value = sourceObject.GetFloatArray(sourceName);
                var pos = new global::System.Numerics.Vector3(value[0], value[1], value[2]);
    ...
    static KVObject ProcessBoneConstraintTarget(KVObject target)
    {
        ...
        AddBoneConstraintProperty<global::System.Numerics.Vector3>(target, node, "m_vOffset", "relative_origin");
        AddBoneConstraintProperty<global::System.Numerics.Quaternion>(target, node, "m_qOffset", "relative_angles");
        return node;
    }
    ...
    static KVObject ProcessBoneConstraintSlave(KVObject slave)
    {
        ...
        AddBoneConstraintProperty<global::System.Numerics.Vector3>(slave, node, "m_vBasePosition", "relative_origin");
        AddBoneConstraintProperty<global::System.Numerics.Quaternion>(slave, node, "m_qBaseOrientation", "relative_angles");
        return node;
    }
    ...
    static void ProcessBoneConstraintChildren(KVObject boneConstraint, KVObject node)
    {
        ...
        if (node.GetStringProperty("_class") == "AnimConstraintParent")
        {
            ...
            var rot = new global::System.Numerics.Quaternion(rotArray[0], rotArray[1], rotArray[2], rotArray[3]);
            var angles = ToEulerAngles(rot);
            angles = new global::System.Numerics.Vector3(angles.Z, angles.X, angles.Y);
    ...
    static KVObject ProcessBoneConstraint(KVObject boneConstraint)
    {
        ...
        AddBoneConstraintProperty<global::System.Numerics.Quaternion>(boneConstraint, node, "m_qAimOffset", "aim_offset");
        AddBoneConstraintProperty<global::System.Numerics.Vector3>(boneConstraint, node, "m_vUpVector", "up_vector");
        AddBoneConstraintProperty<long>(boneConstraint, node, "m_nUpType", "up_type");
        AddBoneConstraintProperty<global::System.Numerics.Quaternion>(boneConstraint, node, "m_qParentBindRotation", "parent_bind_rotation");
        AddBoneConstraintProperty<global::System.Numerics.Quaternion>(boneConstraint, node, "m_qChildBindRotation", "child_bind_rotation");
    ...
    internal static global::System.Numerics.Vector3 ToEulerAngles(global::System.Numerics.Quaternion q)
    {
        // Normalized double intermediates avoid catastrophic cancellation at pitch
        // +/-90 degrees. At the pole, choose roll=0 and retain the combined yaw.
        double x = q.X, y = q.Y, z = q.Z, w = q.W;
        var norm = x * x + y * y + z * z + w * w;
        if (!double.IsFinite(norm) || norm < 1e-20) throw new InvalidDataException("Invalid bone rotation.");
        var k = 2 / norm;
        var sinp = k * (w * y - z * x);
        var pole = Math.Abs(sinp) > .9999995;
        var pitch = pole ? Math.CopySign(Math.PI / 2, sinp) : Math.Asin(sinp);
        var yaw = pole ? Math.Atan2(k * (w * z - x * y), 1 - k * (x * x + z * z))
            : Math.Atan2(k * (w * z + x * y), 1 - k * (y * y + z * z));
        var roll = pole ? 0 : Math.Atan2(k * (w * x + y * z), 1 - k * (x * x + y * y));
        return new global::System.Numerics.Vector3((float)pitch, (float)yaw, (float)roll) * (180 / MathF.PI);
    }
    ...
    public string ToValveModel()
    {
        ...
        if (Translation != global::System.Numerics.Vector3.Zero)
        {
