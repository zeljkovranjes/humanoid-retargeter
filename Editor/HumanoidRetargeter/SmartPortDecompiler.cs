...
{
	internal static async Task<string> RecoverAsync( Asset asset, string folder, CancellationToken token )
	{
		...
		return await Task.Run( () => CompiledAssetRecovery.Recover( input, assetPath, root, folder, paths, token ), token );
	}
}
